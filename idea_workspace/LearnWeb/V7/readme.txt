首先完成一个工作,可以根据浏览器地址栏中通过改变抽象路径部分请求到服务端webapps下对应网络应用的
某个页面.
思路:
...
3:在ClientHandler处理请求的环节,当根据抽象地址去webapps下定义资源后(实例化File对象),添加分支
  如果该资源存在则响应,如果该资源不存在或者定位到的是一个目录,则相应404.
404的响应要做几个变化:
